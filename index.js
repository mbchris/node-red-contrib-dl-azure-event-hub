module.exports = function (RED) {
    ...
    function dlEventHubSend(config) {
        ...
        node.on('input', async function (msg) {
            ...
            node.debug("connecting the producer client...");
            node.status({
                fill: 'yellow',
                shape: 'dot',
                text: "connecting..."
            ...
            if(DEBUG === true) {
                node.debug("open the producerClient connection.");
                node.debug("producerClient object:");
            ...
            try {
                ...
                var msgJSON;
                node.debug("create empty batch with following options:");
                node.debug(batchOptions);
                if(DEBUG === true) {
                ...
                if (typeof (msg.payload) != "string") {
                    ...
                    node.debug("Is String...converting");
                ...
                node.debug("try to add the following event to the batch:");
                ...
                if( isAdded === false ) {
                    ...
                    node.debug(warnText);
                    if(DEBUG === true) {
                        node.debug("adding failed.");
                    }
                }

                //send batch
                await producerClient.sendBatch(batch);
                node.debug("sent batch to event hub.");
                node.status({
                    fill: 'blue',
                    shape: 'dot',
                    text: "sent message"
            ...
            node.debug("Disconnect producer client.");
            node.status({
                fill: 'grey',
                shape: 'dot',
